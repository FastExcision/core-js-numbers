...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const sqRange1 = (x1 - x2) ** 2;
  const sqRange2 = (y1 - y2) ** 2;
  return Math.sqrt(sqRange1 + sqRange2);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalXY = x1 * x2 + y1 * y2;
  const modXY = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(scalXY / modXY);
}
...
function isPrime(n) {
  ...
  if (n < 2) {
    ...
    while (n % checkWhile !== 0) {
      checkWhile += 1;
      checkResult = checkWhile;
    }
    if (checkResult !== n) {
      result = false;
    } else {
...
function getFibonacciNumber(index) {
  ...
  if (index === 0) {
    ...
    for (let i = 3; i <= index; i += 1) {
      fibNumberResult = fibNumberI1 + fibNumberI2;
      fibNumberI2 = fibNumberI1;
      fibNumberI1 = fibNumberResult;
    }
  }
  return fibNumberResult;
}
...
function isPowerOfTwo(num) {
  let degreeCheck = 2;
  for (let i = 1; degreeCheck < num; i += 1) {
    degreeCheck = 2 ** (2 + i - 1);
  }
  if (degreeCheck === num) {
    degreeCheck = true;
...
function getCountOfOddNumbers(number) {
  let numOdd = 0;
  if (number < 0) {
    numOdd = Math.floor(number / 2);
  } else {
    numOdd = Math.ceil(number / 2);
  }
  return Math.abs(numOdd);
}
